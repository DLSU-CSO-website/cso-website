...
export const POST = async (request: Request) => {
  try {
    ...
    if (data.get("image")) {
      ...
      const newAnnouncement = await Announcement.createAnnouncement(
...
export const PUT = async (request: Request) => {
  ...
  try {
    ...
    if (data.get("image")) {
      ...
      const newAnnouncement = await Announcement.editAnnouncement(
        String(data.get("title")),
        String(data.get("body")),
        String(data.get("id")),
        result.secure_url,
      ...
      return NextResponse.json(
        {
          data: newAnnouncement,
          message:
            String(data.get("title")) + " Announcement successfully edited!",
        },
        { status: 200 },
      );
    }
